...
import { useMascotasList } from "../hooks/useMascotasList";
...
function MascotasList() {
  ...
  const canCreate = mascotasConfig.roleAccess.create.includes(roleId);
  const canViewDetail = mascotasConfig.roleAccess.list.includes(roleId);
  const canEdit = mascotasConfig.roleAccess.edit.includes(roleId);
  const canToggle = mascotasConfig.roleAccess.toggle.includes(roleId);

  return (
    <section className="page">
      <div className="page-header">
        <h2>Mascotas - Listado</h2>
        <div className="actions">
          <button className="secondary-btn" onClick={reload} disabled={loading || toggleLoading}>
            Recargar
          </button>
          {canCreate ? (
            <Link className="primary-btn" to="/mascotas/crear">
              Crear
            </Link>
          ) : null}
        </div>
      ...
      <MascotasFilters filters={filters} onFilterChange={updateFilter} onClearFilters={clearFilters} />
      ...
      {!loading && !error ? (
        <>
          <MascotasTable
            ...
            canEdit={canEdit}
            canToggle={canToggle}
            onToggleStatus={toggleMascotaStatus}
          />
